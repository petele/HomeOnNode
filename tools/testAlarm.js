...
function fireAlarm(key, alarm) {
  ...
  var msg = 'alarmFired [' + key + '] at ' + alarm.lastFired;
...
function testAlarmTime(key, alarm) {
  ...
  if (alarm.time[0] === 'R') {
    ...
    if (alarm.time[dayOfWeek] !== '1') {
      return false;
    }
    var newAlarmTime = momentNow.format('YYYY-MM-DD');
    newAlarmTime += alarm.time.substr(alarm.time.indexOf('T'));
    momentAlarm = moment(newAlarmTime);
  } else {
    momentAlarm = moment(alarm.time);
  }
...
function addRandomAlarm() {
  ...
  var randAlarm = {
    ...
    time: timeForAlarm.format(DATE_FORMAT),
    lastFired: 0,
    timesFired: 0,
    enabled: true
  };
